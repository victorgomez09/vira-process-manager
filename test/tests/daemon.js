...
describe('daemon', function () {
    ...
    async function daemonLaunchTest(usePort) {
        ...
        const dprocess = common.trackProcess(await ViraPM.daemon.launch(launchConfig));
        const waitExit = common.awaitEvent(dprocess, 'exit');

        const client = await ViraPM.client.connect(launchConfig['socket']);
    ...
    it('should fail when a daemon is already running', async function () {
        ...
        await expect(common.trackProcess(ViraPM.daemon.launch(launchConfig)))
    ...
    it('should detect dead unix domain sockets', async function () {
        ...
        let dprocess = common.trackProcess(await ViraPM.daemon.launch(launchConfig));
        ...
        dprocess = common.trackProcess(await ViraPM.daemon.launch(launchConfig));
    ...
    inAllModes(function (getDaemon, mode) {
        ...
        it('should restart crashing applications', async function () {
            ...
            // 3. Wait for ViraPM to register that it died
            ...
            // 6. Wait for ViraPM to register that.
            await common.awaitLogLine(client, 'app', 1000,
                { type: 'exit', process: { pid: crashingApp.pid } }
            );
