...
const ConfigError = ViraPM.config.ConfigError;
...
describe('config', function () {
    ...
    async function testConfig(configPath) {
        const config = await ViraPM.config.getConfig(configPath);
    ...
    it('should return the global config when called without a path', async function () {
        const config = await ViraPM.config.getConfig();
    ...
    it('should parse JS and JSON configuration files the same', async function () {
        const JS = await ViraPM.config.getConfig(JS_CONFIG);
        const JS_PROMISE = await ViraPM.config.getConfig(JS_PROMISE_CONFIG);
        const JS_PROMISE_FN = await ViraPM.config.getConfig(JS_PROMISE_FN_CONFIG);
        const JS_FN = await ViraPM.config.getConfig(JS_FN_CONFIG);
        const JSON = await ViraPM.config.getConfig(JSON_CONFIG);
    ...
    it('should normalize configurations', async function () {
        const config1 = await ViraPM.config.getConfig(JS_CONFIG);
        const config2 = await ViraPM.config.getConfig(OTHER1);

        const normalized = ViraPM.config.normalizeArray([config1, config2]);
    ...
    it('should overwrite config values from CLI and ENV', async function () {
        ...
        const ENV = await ViraPM.config.getConfig({
            path: JS_CONFIG,
            env: {
                SOMETHING_ELSE: 'hi',
    ...
    it('should reject malformed config from CLI and ENV', async function () {
        function getBrokenConfigA() {
            return ViraPM.config.getConfig({
                path: JS_CONFIG,
                env: {
    ...
    it('should read config values from a package.json and npm config', async function () {
        ...
        const JS_ARGS_USER = await ViraPM.config.getConfig({
            path: JS_CONFIG,
        ...
        const JS_ENV_USER = await ViraPM.config.getConfig({
            path: JS_CONFIG,
        ...
        const JS_ARGS_GLOBAL = await ViraPM.config.getConfig({
            path: JS_CONFIG,
        ...
        const JS_ENV_GLOBAL = await ViraPM.config.getConfig({
            path: JS_CONFIG,
